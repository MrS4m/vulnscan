...
/**
 * Simulated Network Scanner (NMPA Engine)
 * 
 * Uses a knowledge-based approach to simulate network analysis.
 * Identifies common ports, services, and associated vulnerabilities
 * based on well-known attack patterns and MITRE ATT&CK framework.
 * 
 * NOTE: This does NOT execute real nmap scans. It performs logical
 * analysis based on a curated knowledge base of common services
...
export class NmapNetworkScanner implements INetworkScanner {
    private readonly knowledgeBase: ServiceKnowledge[] = [
        // FTP - Port 21
        {
            port: 21,
            protocol: 'tcp',
    ...
    private readonly quickPorts = [21, 22, 23, 25, 80, 443, 3306, 3389, 8080];
    private readonly fullPorts = [21, 22, 23, 25, 53, 80, 110, 143, 443, 445, 993, 995, 3306, 3389, 5432, 6379, 8080, 8443, 27017];
     ...
     */
    private determineOpenPorts(ip: string, portsToScan: number[]): number[] {
        ...
        for (const p of alwaysOpen) {
            if (portsToScan.includes(p)) openPorts.push(p);
        }
